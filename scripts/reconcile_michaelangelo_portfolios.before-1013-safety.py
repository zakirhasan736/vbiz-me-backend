...
def main():
    ...
    try:
        ...
        # --------------------------------------------------
        section("6. PLAN MISSING ATTACHMENT 1013")
        ...
        with pg.cursor(cursor_factory=RealDictCursor) as c:
            ...
            print(
                f"CREATE Attachment 1013 -> {attachment_id}"
            )
            print(
                f"  parent Portfolio 125 "
                f"-> {portfolio_map[125]}"
            )
            print(
                f"  docName={old_att['doc_name']!r}"
            )
            print(
                "  MEDIA STATUS: source file/url still unresolved"
            )
            ...
            if args.apply:
                ...
                """, (
                    ...
                    old_att["doc_name"],
                    old_att["doc_name"],
                    old_att.get("created_at"),
                    old_att.get("updated_at")
                        or old_att.get("created_at"),
                ))

                if c.rowcount != 1:
                    raise RuntimeError(
                        "Could not insert Attachment 1013 "
                        "(AttachmentType legacyId=5 missing?)"
                    )
