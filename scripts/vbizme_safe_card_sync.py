...
class Sync:
    def __init__(self, args: argparse.Namespace):
        ...
        self.portfolio_map: Dict[int, str] = {}
    ...
    def update(self, table: str, row_id: Any, values: Dict[str, Any], *, context: Dict[str, Any]) -> bool:
        ...
        # profileId is normally protected to prevent accidental
        # reassignment of records between card owners.
        #
        # Existing legacy Attachment rows are the one deliberate
        # exception: some media was migrated to S3 before its
        # Profile/Post existed in PostgreSQL and therefore needs
        # an explicit relationship relink.
        if (
            table == "Attachment"
            and context.get("attachmentRelink") is True
        ):
            protected.discard("profileId")
        ...
        # HARD MEDIA SAFETY:
        # Attachment relationship repairs must never overwrite
        # migrated S3/media metadata already present in NEW.
        if (
            table == "Attachment"
            and context.get("attachmentRelink") is True
        ):
            allowed_relink_fields = {
                "profileId",
                "postId",
                "attachableId",
                "attachableType",
    ...
    def sync_profiles(self):
        ...
        for old in rows:
            ...
            if existing and not existing.get("__multiple__"):
                ...
                # COMPLETENESS PASS:
                # Existing NEW profile content is authoritative.
                # Only fill missing relationship references.
                relationship_fill = {}
                ...
                for key in (
                    ...
                    if (
                        ...
                        relationship_fill[key] = mapped[key]

                if relationship_fill:
                    self.update(
                        "Profile",
                        existing["id"],
                        relationship_fill,
                        context={
                            **context,
                            "completenessRelationshipFill":
                                sorted(relationship_fill.keys()),
                        },
                    )
                else:
                    self.event(
                        "KEEP_NEW",
                        "Profile",
                        reason="completeness_existing_profile_preserved",
                        **context,
                    )
    ...
    def sync_settings(self):
        ...
        for old in rows:
            ...
            if existing and not existing.get("__multiple__"):
                # COMPLETENESS PASS:
                # Never overwrite an existing NEW Setting value.
                self.event(
                    "KEEP_NEW",
                    "Setting",
                    reason="completeness_existing_setting_preserved",
                    **context,
                )
                continue

            # Same profile/key without legacyId is the same semantic Setting.
            # Link the OLD identity only. Never change the NEW value here.
            same_key = self.find_one(
                "Setting",
                {
                    "profileId": new_pid,
                    "key": old.get("key"),
            ...
            if same_key and not same_key.get("__multiple__"):
                ...
                same_legacy = same_key.get("legacyId")

                if same_legacy in (None, int(old["id"])):

                    if same_legacy is None:
                        ...
                        if self.apply:
                            with self.pg.cursor() as cur:
                                ...
                                if cur.rowcount != 1:
                                    raise RuntimeError(
                                        "Setting legacyId link failed "
                                        f"for OLD Setting {old['id']}"
                                    )
                        ...
                        self.event(
                            "KEEP_NEW",
                            "Setting",
                            reason="same_key_already_has_same_legacy_id",
                            **context,
                        )
                ...
                self.event(
                    "SETTING_KEY_CONFLICT",
                    "Setting",
                    rowId=same_key["id"],
                    existingLegacyId=same_legacy,
                    **context,
                )
                continue
    ...
    def sync_services(self):
        ...
        for old in rows:
            ...
            if existing and not existing.get("__multiple__"):
                ...
                if table == "Review":
                    # Existing NEW Review content is authoritative during
                    # completeness migration. Legacy service IDs are not
                    # sufficient proof that the semantic content is identical.
                    self.event(
                        "KEEP_NEW",
                        "Review",
                        reason="completeness_existing_review_preserved",
                        **common_context,
                    )
    ...
    def sync_direct_post(self, table: str, old: Dict[str, Any], new_pid: str, metas: Dict[str, Any]):
        ...
        if existing and not existing.get("__multiple__"):
            ...
            # SAFE_SINGLETON_PROFILE_LINK_V1
            # Some direct-section tables are intentionally one-row-per-profile.
            # If NEW already contains that singleton, preserve NEW content and
            # attach the OLD legacyPostId instead of inserting a duplicate.

            with self.pg.cursor() as _cur:
                _cur.execute(
                    """
                    SELECT EXISTS (
                        SELECT 1
                        ...
                        WHERE schemaname='public'
                          ...
                          AND indexdef LIKE '%%("profileId")%%'
                    )
                    """,
                    (table,),
                )
                _singleton_profile_unique = bool(_cur.fetchone()[0])

            if _singleton_profile_unique:
                _singleton = self.find_one(
                    table,
                    {"profileId": new_pid},
                )

                if _singleton and _singleton.get("__multiple__"):
                    self.event(
                        "SINGLETON_PROFILE_CONFLICT",
                        table,
                        reason="multiple_rows_for_unique_profile_section",
                        **context,
                    )
                    return

                if _singleton:
                    _incoming_legacy = int(old["id"])
                    _existing_legacy = _singleton.get("legacyPostId")

                    if (
                        _existing_legacy is None
                        and "legacyPostId" in self.columns(table)
                    ):
                        if not self.apply:
                            self.event(
                                "WOULD_LINK_SINGLETON_LEGACY_POST_ID",
                                table,
                                rowId=_singleton["id"],
                                legacyPostId=_incoming_legacy,
                                **context,
                            )
                        else:
                            with self.pg.cursor() as _cur:
                                ...
                                if _cur.rowcount != 1:
                                    raise RuntimeError(
                                        f"Singleton legacyPostId link failed "
                                        f"for {table} / OLD post {_incoming_legacy}"
                                    )

                            self.event(
                                "LINK_SINGLETON_LEGACY_POST_ID",
                                table,
                                rowId=_singleton["id"],
                                legacyPostId=_incoming_legacy,
                                **context,
                    ...
                    if (
                        ...
                        and int(_existing_legacy) == _incoming_legacy
                    ):
                        self.event(
                            "KEEP_NEW",
                            table,
                            reason="singleton_same_legacy_post",
                            **context,
                        )
                        return

                    self.event(
                        "SINGLETON_LEGACY_CONFLICT",
                        table,
                        rowId=_singleton["id"],
                        existingLegacyPostId=_existing_legacy,
                        incomingLegacyPostId=_incoming_legacy,
                        **context,
                    )
                    return
    ...
    def sync_post_metas(self):
        ...
        for old in rows:
            profile_id = old.get("__profile_id")

            if (
                profile_id is None
                or not self.wanted_profile(int(profile_id))
            ):
                continue
    ...
    def report_attachment_delta(self):
        ...
        for old in rows:
            ...
            elif "Portfolio" in old_type:

                attachable_id = (
                    self.portfolio_map.get(
                        legacy_attachable_id
                    )
                )

                if not attachable_id:
                    portfolio = self.find_one(
                        "Portfolio",
                        {
                            "legacyId":
                                legacy_attachable_id
                    ...
                    if (
                        ...
                        attachable_id = portfolio[
                            "id"
                        ]

            elif "Service" in old_type:

                # Preserve the OLD service ID.
                #
                # Existing vBiz Me review-media repair
                # logic uses:
                #
                # Attachment.attachableId
                #      ==
                # Review.legacyServiceId
                #
                # so this relationship must remain
                # numeric/legacy rather than replacing
                # it with a Review UUID.
                attachable_id = str(
                    legacy_attachable_id
                )
            ...
            if not attachable_id:
                self.event(
                    "SKIP_ATTACHMENT_TARGET_MISSING",
                    "Attachment",
                    legacyId=legacy_attachment_id,
                    attachableType=old_type,
                    attachableLegacyId=
                        legacy_attachable_id,
                    profileLegacyId=
                        profile_legacy_id,
            ...
            values = {
                ...
                # Keep the legacy filename here.
                # migrate-media-by-profile.ts uses
                # docName/profile legacyId to locate
                # the actual legacy source URL and
                # changes url only after successful
                # S3 upload.
                "url":
                    doc_name,
            ...
            if existing_attachment:
                ...
                # RELATIONSHIPS ONLY.
                #
                # Never overwrite existing media URL, publicId,
                # resourceType, format, bytes, extension or mimeType.
                #
                # These may already point to successfully migrated
                # S3 objects and are authoritative.
                ...
                needs_relink = any(
                    existing_attachment.get(k) != v
                    for k, v in relink_values.items()
                )
                ...
                if needs_relink:
                    ...
                    self.event(
                        ...
                        "Attachment",
                        reason=
                            "attachment_relationship_already_correct",
                        **context,
                    )
